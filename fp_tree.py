...
def makeFlistGPU(d_offsets, d_transactions, d_flist, num_transactions, all_items_in_transactions):
    ...
    private_items = cuda.shared.array(MAX_UNIQUE_ITEMS, uint32)
    ...
    for i in range(d_offsets[index], item_ends):
        if d_transactions[i] >= 0 and d_transactions[i] < MAX_UNIQUE_ITEMS:
...
def make_fp_tree():
    ...
    offsets, transactions, num_transactions, all_items_in_transactions = readFile("big_data.txt")
    ...
    #d_flist = cuda.device_array(MAX_UNIQUE_ITEMS, dtype=np.uint32)
